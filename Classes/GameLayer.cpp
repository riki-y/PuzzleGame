...
bool GameLayer::onTouchBegan(Touch* touch, Event* unused_event)
{
    ...
    if (_movingBall) {
        ...
        // どのボールにもタップしていない場合は、今後のタップイベントを取得しない
        return false;
    }
}
                ...
                if (num >= 2) {
                    ...
                    for (int i = 0; i <= num; i++) {
                        ...
                        if (linedBall->getRemovedNo() > 0) {
                            // すでにRemovedNoがあるものが存在するので、removedNoを取得して次の処理を行う
...
void GameLayer::initEnemy()
{
    ...
    // 敵の表示
    _enemy = Sprite::create("Niwango.png");
...
void GameLayer::initMembers()
{
    ...
    for (int i = 0; i < fileNames.size(); i++) {
        ...
        member->setPosition(Point(70 + i * 125, 594));
